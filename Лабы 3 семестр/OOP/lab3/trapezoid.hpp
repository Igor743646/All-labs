...
Trapezoid::Trapezoid(Point cen, double a, double b, double c): Figure(cen, 4), base1(a), base2(b), height(c)  {
	if ((base1 < 0) || (height < 0) || (base2 < 0)) throw "Основания и высота должны быть положительными!";
	CreatePoints();
}
...
std::istream& operator>> (std::istream& in, Trapezoid* A) {
	...
	in >> A->height;
	if ((A->base1 < 0) || (A->height < 0) || (A->base2 < 0)) throw "Основания и высота должны быть положительными!";
	A->CreatePoints();
	return in;
}
