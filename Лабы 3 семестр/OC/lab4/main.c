...
const char* mmaped_file_for_numbers = "p1";
const char* mmaped_file_for_flag = "p2";
const char* mmaped_mutex_one = "mu1";
const char* mmaped_mutex_two = "mu2";

void mutexInit(pthread_mutex_t *mt) {
	pthread_mutexattr_t attr;
	if (pthread_mutexattr_init(&attr) != 0) {
		perror("mutex attribute init");
		exit(EXIT_FAILURE);
	}
	if (pthread_mutexattr_setpshared(&attr, PTHREAD_PROCESS_SHARED) != 0) {
		perror("mutex attribute setpshered");
		exit(EXIT_FAILURE);
	}
	if (pthread_mutex_init(mt, &attr) != 0) {
		perror("mutex init");
		exit(EXIT_FAILURE);
	}
}
...
int main() {
	...
	if (pthread_mutex_lock(m1) != 0) error_msg_with_free_fileName("mutex lock");
	if (pthread_mutex_lock(m2) != 0) error_msg_with_free_fileName("mutex lock");
	...
	if (id == -1) {
		...
		while (scanf("%d%c", &x, &endln)>0) {
			...
			if (pthread_mutex_unlock(m1) != 0) error_msg_with_free_fileName("mutex unlock");
			if (pthread_mutex_lock(m2) != 0) error_msg_with_free_fileName("mutex lock");
